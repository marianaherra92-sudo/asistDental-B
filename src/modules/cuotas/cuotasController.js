...
const CuotasController = {
    ...
    createManyCuotas: async (req, res) => {
        try {
            const { cuotas } = req.body;

            if (!Array.isArray(cuotas)) {
                return res.status(400).json({ error: 'El formato de cuotas es inválido' });
            }

            await CuotasService.createManyCuotas(cuotas);
    ...
    updateCuota: async (req, res) => {
        try {
            const cuota = await CuotasService.updateCuota(
                req.params.id_cuota,
                req.body
            );
            res.json(cuota);
